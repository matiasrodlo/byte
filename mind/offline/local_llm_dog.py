"""
RobotDog Local LLM Control System
================================
Main script for controlling RobotDog using a local LLM with voice/text input.
"""
...
# --- Argument Parsing ---
parser = argparse.ArgumentParser(description="Control RobotDog with a local LLM.")
parser.add_argument(
    '--input_mode',
    type=str,
    default='voice',
...
try:
    ...
    print(f"\033[31mERROR: Failed to initialize RobotDog: {e}\033[m") # Added error print
    # Allow running without physical robotdog for testing LLM part if needed
    # For now, we exit if RobotDog fails.
    sys.exit(1)
...
if __name__ == "__main__":
    try:
        ...
        if 'my_dog' in locals() and my_dog is not None:
            print("Closing RobotDog connection...")
            my_dog.close()
