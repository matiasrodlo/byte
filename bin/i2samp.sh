#!/bin/bash
"""
RobotDog Audio Configuration Script
==================================
Script for configuring audio settings on RobotDog with RobotHat.
This script sets up I2S audio drivers and PulseAudio configuration.
"""
...
SOFTVOL_SPEAKER_NAME="RobotDog RobotHat speaker"
SOFTVOL_MIC_NAME="RobotDog RobotHat mic"
...
sudocheck() {
    if [ $(id -u) -ne 0 ]; then
        warning "Install must be run as root. Try 'sudo bash ./bin/i2samp.sh'"
        exit 1
    fi
}
...
check_robothat() {
    # find RobotDog RobotHat device-tree directory
    ...
    if [[ -z "${hat_dir}" ]]; then
        echo "No RobotDog RobotHat 5 found"
        return
    fi

    # read RobotDog RobotHat info
    ...
    success "Found RobotDog RobotHat:"
...
install_soundcard_driver() {
    info "install RobotDog RobotHat soundcard driver >>>"
    ...
    # detect RobotDog RobotHat 5
    # =====================================
    newline
    info "check RobotDog RobotHat 5 ..."
    check_robothat
    ...
    success "RobotDog RobotHat speaker enable GPIO: ${robothat_spk_en}"
    success "RobotDog RobotHat has microphone: ${_is_with_mic}"
    ...
    # --- Done ---
    newline
    success "RobotDog audio configuration complete!"
    newline
}
